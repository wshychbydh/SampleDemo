...
public class PinchImageView extends AppCompatImageView {
   ...
   * 获取内部变换矩阵.
   *
   * 内部变换矩阵是原图到fit center状态的变换,当原图尺寸变化或者控件大小变化都会发生改变 当尚未布局或者原图不存在时,其值无意义.所以在调用前需要确保前置条件有效,否则将影响计算结果.
   *
   * @param matrix 用于填充结果的对象
   * @return 如果传了matrix参数则将matrix填充后返回, 否则new一个填充返回
   */
   ...
   * 获取图片总变换矩阵.
   *
   * 总变换矩阵为内部变换矩阵x外部变换矩阵,决定了原图到所见最终状态的变换 当尚未布局或者原图不存在时,其值无意义.所以在调用前需要确保前置条件有效,否则将影响计算结果.
   *
   * @param matrix 用于填充结果的对象
   * @return 如果传了matrix参数则将matrix填充后返回, 否则new一个填充返回
   * @see #getOuterMatrix(Matrix)
   ...
   * 获取图片最大可放大的比例
   *
   * 如果放大大于这个比例则不被允许. 在双手缩放过程中如果图片放大比例大于这个值,手指释放将回弹到这个比例. 在双击放大过程中不允许放大比例大于这个值.
   ...
   * 计算双击之后图片接下来应该被缩放的比例
   *
   * 如果值大于getMaxScale或者小于fit center尺寸，则实际使用取边界值. 通过覆盖此方法可以定制不同的图片被双击时使用不同的放大策略.
   *
   ...
   * 缩放模式下图片的缩放中点.
   *
   * 为其指代的点经过innerMatrix变换之后的值. 其指代的点在手势过程中始终跟随mLastMovePoint. 通过双指缩放时,其为缩放中心点.
   *
   ...
   * 双击后放大或者缩小
   *
   * 将图片缩放比例缩放到nextScale指定的值. 但nextScale值不能大于最大缩放值不能小于fit center情况下的缩放值. 将双击的点尽量移动到控件中心.
   *
   * @param x 双击的点
   * @param y 双击的点
  ...
  private static abstract class ObjectsPool<T> {
     ...
     * 获取一个空闲的对象
     *
     * 如果对象池为空,则对象池自己会new一个返回. 如果对象池内有对象,则取一个已存在的返回. take出来的对象用完要记得调用given归还.
  ...
  public static class MathUtils {
     ...
     * 计算点除以矩阵的值
     *
     * matrix.mapPoints(unknownPoint) -> point 已知point和matrix,求unknownPoint的值.
     *
     * @return unknownPoint
     */
