...
class AsyncPack {
    ...
    fun <T1, T2, T> zip(R1: (callback: (data: T1) -> Unit) -> Unit,
                        R2: (callback: (data: T2) -> Unit) -> Unit): Executor<T1, T2, T> {
        val executeQueue = AsyncPack.Executor<T1, T2, T>()
        executeQueue.zip(R1, R2)
        return executeQueue
    }

    fun <T1, T2, T3, T> zip(R1: (callback: (data: T1) -> Unit) -> Unit,
                            R2: (callback: (data: T2) -> Unit) -> Unit,
                            R3: (callback: (data: T3) -> Unit) -> Unit) {

    }

    fun <D, T> zip(vararg R: (callback: (data: D) -> Unit) -> Unit): AsyncPack.ExecuteQueue<D, T> {
        val executeQueue = AsyncPack.ExecuteQueue<D, T>()
        executeQueue.zip(*R)
        return executeQueue
    }
    ...
    class Executor<T1, T2, T> internal constructor() {
         ...
         * run on sub thread
         */
        fun zip(R1: (callback: (data: T1) -> Unit) -> Unit,
                R2: (callback: (data: T2) -> Unit) -> Unit): Executor<T1, T2, T> {
            this.R1 = R1
            this.R2 = R2
            return this
